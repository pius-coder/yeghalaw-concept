...
<div
	class="flex w-full flex-col gap-6 rounded-[10px] border border-white/80 bg-[linear-gradient(145deg,#ffffff_0%,#fff9f2_48%,#edf8f4_100%)] p-5 shadow-[inset_3px_3px_47px_5px_rgba(255,255,255,0.65)]"
>
	...
	{#if landscapeSrc || portraitSrc}
		<div class="flex flex-wrap gap-2.5">
			...
			<!-- {#if portraitSrc}
				<div class="min-w-[200px] flex-1 overflow-hidden rounded-[10px]">
					<img src={portraitSrc} alt="" class="aspect-[4/3] h-full w-full object-cover" />
				</div>
			{/if} -->
		</div>
	{/if}
